(function(){var e=`omnichat_widget_visitor_id`;function t(){let e=document.currentScript;if(e)return e;let t=document.getElementsByTagName(`script`);return t[t.length-1]}function n(){let t=window.localStorage.getItem(e);return t||(t=crypto.randomUUID(),window.localStorage.setItem(e,t)),t}async function r(e,t,n,r){let i=await fetch(`https://creativeconnects.web.id/api/widget${n}`,{...r,headers:{"Content-Type":`application/json`,"X-Widget-Key":e,"X-Visitor-Id":t,...r?.headers}});if(!i.ok)throw Error(`Widget request failed: ${i.status}`);return i.json()}function i(){let e=document.createElement(`style`);e.textContent=`
  ...
  `,document.head.appendChild(e)}function a(e,t,n){let r=document.createElement(`div`);if(r.className=`omnichat-widget-bubble ${t.direction===`inbound`?`contact`:`bot`}`,r.textContent=t.text,r.dataset.messageId=t.id,e.appendChild(r),t.buttons&&t.buttons.length>0&&n){let r=document.createElement(`div`);r.className=`omnichat-widget-buttons`,t.buttons.forEach(e=>{let t=document.createElement(`button`);t.className=`omnichat-widget-option`,t.textContent=e.label,t.addEventListener(`click`,()=>{Array.from(r.querySelectorAll(`button`)).forEach(e=>e.disabled=!0),n(e)}),r.appendChild(t)}),e.appendChild(r)}return e.scrollTop=e.scrollHeight,r}async function o(){let e=t().dataset.widgetKey;if(!e){console.error(`Creative Connects widget: missing data-widget-key attribute`);return}i();let o=n(),s=new Set,c=[],l=null,u=document.createElement(`button`);u.className=`omnichat-widget-launcher`,u.setAttribute(`aria-label`,`Open chat`),u.textContent=`💬`;let d=document.createElement(`div`);d.className=`omnichat-widget-panel`;let f=document.createElement(`div`);f.className=`omnichat-widget-header`,f.textContent=`Chat with us`;let p=document.createElement(`div`);p.className=`omnichat-widget-messages`;let m=document.createElement(`div`);m.className=`omnichat-widget-input-row`;let h=document.createElement(`input`);h.className=`omnichat-widget-input`,h.placeholder=`Type a message…`;let g=document.createElement(`button`);g.className=`omnichat-widget-send`,g.textContent=`Send`,m.appendChild(h),m.appendChild(g),d.appendChild(f),d.appendChild(p),d.appendChild(m),document.body.appendChild(u),document.body.appendChild(d);let _=!1;function v(e){s.has(e.id)||(e.direction===`inbound`&&c.length>0&&c.shift().element.remove(),s.add(e.id),l=e.id,a(p,e,e=>S(e.id,e.label,e.label)))}async function y(){if(l)try{let{data:t}=await r(e,o,`/messages?since=${encodeURIComponent(l)}`);t.forEach(v)}catch{}}async function b(){if(!_){_=!0;try{let{data:t}=await r(e,o,`/bootstrap`);t.messages.length===0&&t.welcomeMessage&&a(p,{id:`welcome`,direction:`outbound`,text:t.welcomeMessage,timestamp:new Date().toISOString()}),t.messages.forEach(v),setInterval(y,4e3)}catch{_=!1}}}let x=!1;async function S(t,n,i){if(x||!t)return;x=!0,h.disabled=!0,g.disabled=!0;let s=a(p,{id:`pending-${Date.now()}`,direction:`inbound`,text:n??t,timestamp:new Date().toISOString()});c.push({text:n??t,element:s});try{let{data:n}=await r(e,o,`/messages`,{method:`POST`,body:JSON.stringify({text:t,buttonLabel:i})});v(n.message)}catch{}finally{x=!1,h.disabled=!1,g.disabled=!1,h.focus()}}async function C(){let e=h.value.trim();e&&(h.value=``,await S(e))}g.addEventListener(`click`,C),h.addEventListener(`keydown`,e=>{e.key===`Enter`&&(e.preventDefault(),C())}),u.addEventListener(`click`,()=>{d.classList.toggle(`open`)&&b()})}document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,o):o()})();