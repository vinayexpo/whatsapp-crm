...
import type { TeamMember, VoiceCall } from "~/data/types";
import { formatDate, formatTime } from "~/utils/format";

export function VoiceAgentFollowupQueue() {
  ...
  function reload() {
    setLoading(true);
    return apiClient
      .listVoiceCalls({ needsHumanFollowup: true })
      .then(setCalls)
      .catch(() => {
  ...
  async function handleAssign(callId: string, userId: string | null) {
    ...
    try {
      const updated = await apiClient.assignVoiceCallFollowup(callId, userId);
  ...
  async function handleComplete(callId: string) {
    setUpdatingId(callId);
    try {
      await apiClient.completeVoiceCallFollowup(callId);
