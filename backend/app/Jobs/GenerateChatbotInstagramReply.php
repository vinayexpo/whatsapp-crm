...
class GenerateChatbotInstagramReply implements ShouldQueue
{
    ...
    public function handle(ChatbotReplyServiceInterface $replyService, MessagingDriverResolver $resolver): void
    {
        ...
        if (! $conversation || ! $inboundMessage || $conversation->channel !== 'instagram') {
            return;
        }
        ...
        $chatbot = Chatbot::query()
            ->where('status', 'active')
            ->whereJsonContains('channels', 'instagram')
            ->first();
